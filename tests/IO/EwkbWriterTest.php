...
class EwkbWriterTest extends EwkbAbstractTestCase
{
    ...
    public static function providerWrite() : \Generator
    {
        foreach (self::providerLittleEndianEwkb() as [$wkt, $ewkb]) {
            yield [$wkt, $ewkb, ByteOrder::LittleEndian];
        }
        ...
        foreach (self::providerBigEndianEwkb() as [$wkt, $ewkb]) {
            yield [$wkt, $ewkb, ByteOrder::BigEndian];
        }
    ...
    public static function providerWriteEmptyPointWithNanSupport() : array
    {
        return [
            ...
            [Point::xyzEmpty(), ByteOrder::BigEndian, '00800000017ff80000000000007ff80000000000007ff8000000000000'],
            [Point::xyzEmpty(), ByteOrder::LittleEndian, '0101000080000000000000f87f000000000000f87f000000000000f87f'],
            [Point::xymEmpty(), ByteOrder::BigEndian, '00400000017ff80000000000007ff80000000000007ff8000000000000'],
            [Point::xymEmpty(), ByteOrder::LittleEndian, '0101000040000000000000f87f000000000000f87f000000000000f87f'],
            [Point::xyzmEmpty(), ByteOrder::BigEndian, '00c00000017ff80000000000007ff80000000000007ff80000000000007ff8000000000000'],
            [Point::xyzmEmpty(), ByteOrder::LittleEndian, '01010000c0000000000000f87f000000000000f87f000000000000f87f000000000000f87f'],
        ];
    }
}
